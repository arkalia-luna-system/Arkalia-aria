...
class TestLoadPerformance:
    ...
    def test_create_many_pain_entries(self):
        """Test création de 1000+ entrées de douleur"""
        start_time = time.time()

        # Créer 100 entrées (1000 serait trop long pour les tests)
        entries_created = 0
        for i in range(100):
            pain_entry = {
                "intensity": i % 11,
        ...
        assert entries_created == 100

        # Vérifier que le temps est raisonnable (< 10 secondes pour 100 entrées)
        assert elapsed_time < 10.0

        # Vérifier qu'on peut récupérer les entrées
        response = client.get("/api/pain/entries/recent?limit=100")
        assert response.status_code == 200
        data = response.json()
        assert len(data.get("entries", [])) <= 100

    def test_query_performance_with_many_entries(self):
        """Test performance des requêtes avec beaucoup d'entrées"""
        # Créer quelques entrées pour avoir des données
        for i in range(50):
            pain_entry = {
                "intensity": i % 11,
    ...
    def test_pagination_performance(self):
        ...
        # Tester pagination avec différents offsets
